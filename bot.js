...
// 🧹 GARBAGE COLLECTOR (The 5-Minute Auto-Delete Feature)
// This runs every 60 seconds to check for old files
setInterval(() => {
      ...
      fs.stat(filePath, (err, stats) => {
        ...
        if (fileAge > fiveMinutes) {
          fs.unlink(filePath, (err) => {
            if (!err) console.log(`🧹 Auto-Cleaned: ${file}`);
          });
        }
      });
    });
...
async function generateKHQRCard(qrText, name, currencyType) {
  ...
  ctx.fillText("Powered by Bakong", width / 2, 870);
...
const setupBot = (token, domain, createTempLink, io) => {
  ...
  const backMenu = {
    reply_markup: {
      inline_keyboard: [[{ text: "🔙 Back", callback_data: "cmd_start" }]],
    },
  };
...
• <b>Reels</b> → FB & Instagram 📸
    ...
    if (text === "/help")
      return bot.sendMessage(
        chatId,
        "📌 <b>Simple Guide:</b>\n\nJust <b>Paste a Link</b>.\nI will auto-detect the best format for you.",
        { parse_mode: "HTML", ...backMenu },
      );
    if (text === "/contact")
      return bot.sendMessage(chatId, "📞 <b>Support:</b> @Tutuvid", {
        parse_mode: "HTML",
      });
    if (text === "/source")
      return bot.sendMessage(
        chatId,
        `👨‍💻 <a href="${GITHUB_LINK}">View Source Code</a>`,
        { parse_mode: "HTML" },
      );
  ...
  async function processLink(chatId, url, firstName) {
    ...
    // 🧠 Auto-Audio Detection for YouTube
    ...
    // 1. Initial Status
    const msg = await bot.sendMessage(chatId, `🔎 <b>Analyzing Link...</b>`, {
      parse_mode: "HTML",
    });
    ...
    try {
      ...
      bot
        .editMessageText(`⬇️ <b>Downloading...</b>`, {
          chat_id: chatId,
      ...
      if (data.status === "success" && fs.existsSync(finalPath)) {
        await bot.editMessageText(`📤 <b>Uploading...</b>`, {
          chat_id: chatId,
          message_id: msg.message_id,
          parse_mode: "HTML",
        });
        ...
        // ✨ IMMEDIATE CLEANUP (Primary Strategy)
        // We delete right after sending to keep Render disk clear.
        // If this fails, the 5-minute Garbage Collector (at the top of file) will catch it later.
        try {
      ...
      bot.sendMessage(chatId, "❌ <b>Server Busy.</b> Try again later.", {
        parse_mode: "HTML",
      });
      // Cleanup on error
      try {
  ...
  async function updateStats(chatId, name, platform) {
    ...
    await User.updateOne(
      { telegramId: chatId },
      { $set: { lastActive: Date.now(), firstName: name }, $inc: inc },
      { upsert: true },
    );
  }
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (action === "cmd_start")
      bot.editMessageText(`👋 <b>Welcome back!</b>\nPaste a link to start.`, {
        chat_id: chatId,
      ...
      try {
        ...
        if (khqr.status.code === 0) {
          ...
          bot.sendPhoto(
            ...
            card,
            {
              caption: `💸 <b>Donate Support</b>\nScan with ABA/Bakong to support the server!`,
              parse_mode: "HTML",
            },
            { filename: "donate.png" },
          );
